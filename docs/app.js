...
(function registerServiceWorker() {
  ...
  window.addEventListener('load', async () => {
    try {
      const registration = await navigator.serviceWorker.register('./sw.js', {
        scope: './',
        updateViaCache: 'none' // ← IMPORTANT pour updates
      });
      
      console.log('[SW] ✅ Enregistré:', registration.scope);
      
      // ✅ NOUVEAU: Register Protocol Handler
      if ('registerProtocolHandler' in navigator) {
        try {
          navigator.registerProtocolHandler(
            'web+mijoro',
            location.origin + '/?action=protocol&url=%s',
            'Mijoro Boutique'
          );
  ...
  function showUpdateNotification(worker) {
    const shouldUpdate = confirm(
      '🔄 Misy version vaovao miaraka amin\'ny features vaovao!\n\n' +
      '✨ File Handlers\n' +
      '✨ Deep Linking\n' +
      '✨ Widgets\n\n' +
...
async function loadSlides() {
  try {
    console.log('[loadSlides] 🔄 Starting...');
    ...
    if (typeof ensureSupabase === 'function') {
      try {
        ...
        if (error) {
          console.error('[loadSlides] ⚠️ Supabase error:', error);
        } else if (data && data.length > 0) {
          console.log('[loadSlides] ✅ Loaded from Supabase:', data.length);
          
          // Map to app format
          const slides = data.map(row => ({
            id: row.id,
            image: row.image_url,
            alt: row.alt_text || 'Slide',
            link: row.link_url || '#'
          }));
        ...
        console.warn('[loadSlides] ⚠️ Supabase failed, using fallback');
      }
    }
    ...
    if (raw) {
      ...
      if (Array.isArray(arr) && arr.length) {
        console.log('[loadSlides] ✓ Loaded from localStorage:', arr.length);
        return arr;
      }
    }
    
    // 3. Final fallback: default
    console.log('[loadSlides] Using default slides');
...
async function saveSlides(slides) {
  try {
    ...
    localStorage.setItem(SLIDES_KEY, JSON.stringify(slides));
    console.log('[saveSlides] ✓ Saved to localStorage:', slides.length);
    ...
    if (typeof ensureSupabase === 'function') {
      try {
        ...
        const { error: deleteError } = await sb
          .from('slideshow_images')
          .delete()
          .gt('position', -1); // Delete all (trick: position > -1)
        
        if (deleteError) {
          console.error('[saveSlides] ⚠️ Delete error:', deleteError);
        } else {
          console.log('[saveSlides] ✓ Old slides deleted');
        }
        ...
        if (slides.length > 0) {
          ...
          if (insertError) {
            console.error('[saveSlides] ❌ Insert error:', insertError);
          } else {
            console.log('[saveSlides] ✅ Synced to Supabase:', rows.length, 'slides');
          }
        }
        
      } catch (syncErr) {
        console.error('[saveSlides] ⚠️ Supabase sync failed:', syncErr);
        // Ne bloque pas, localStorage est déjà OK
      }
    }
...
document.addEventListener('DOMContentLoaded', async function() {
  try {
    ...
    if (typeof loadSlides === 'function') {
      window.slidesData = await loadSlides();
      console.log('[Slideshow] ✓ Loaded:', window.slidesData.length, 'slides');
    }
    ...
    // Fallback
    window.slidesData = SLIDES_DEFAULT.slice();
    renderSlideshow();
  }
});
...
async function submitSlide() {
  ...
  try {
    ...
    console.log('[submitSlide] 📤 Uploading slide...');
    ...
    console.log('[submitSlide] ✅ Uploaded:', publicUrl.data.publicUrl);
    ...
    // ✅ AWAIT sync to Supabase
    await saveSlides(window.slidesData);
    
    console.log('[submitSlide] ✅ Slide saved');
...
async function deleteSlide(id) {
  ...
  try {
    console.log('[deleteSlide] 🗑️ Deleting:', id);
    
    // Filter out deleted slide
    var filtered = [];
    ...
    console.log('[deleteSlide] Remaining:', window.slidesData.length);
    
    // ✅ AWAIT sync to Supabase
    await saveSlides(window.slidesData);
    
    console.log('[deleteSlide] ✅ Synced to Supabase');
...
// ==========================================
// AGENT MIORA - UNIVERSAL AI ASSISTANT
// Version 3.0 - Pure Agent
// ==========================================

console.log('🚀 [Agent Miora] Starting initialization...');
...
(function initGlobalUtils() {
  ...
  console.log('[Agent Utils] 🔧 Initializing...');
  
  // Language detection
  window.detectLanguage = function(text, currentLanguage = 'mg') {
    const msg = text.toLowerCase().trim();
    
    if (msg.length < 3) return currentLanguage;
    
    // English detection
    const englishKeywords = [
      'tell', 'what', 'how', 'where', 'when', 'why', 'who',
      'show', 'find', 'search', 'help', 'please', 'the', 'is', 'are'
    ];
    
    let enCount = 0;
    const words = msg.split(/\s+/);
    words.forEach(word => {
      if (englishKeywords.includes(word)) enCount++;
    });
    
    if (enCount >= 3) return 'en';
    
    // French detection
    const frenchKeywords = [
      'je', 'tu', 'le', 'la', 'les', 'un', 'une', 'des',
      'comment', 'quoi', 'quel', 'où', 'cherche', 'trouve', 'bonjour', 'merci'
    ];
    
    let frCount = 0;
    words.forEach(word => {
      if (frenchKeywords.includes(word)) frCount++;
    });
    ...
    const malagasyKeywords = [
      'aho', 'ianao', 'dia', 'ny', 'amin', 'fa', 'misy', 'tsy',
      'ahoana', 'inona', 'mitady', 'manao', 'salama', 'misaotra'
    ];
    
    let mgCount = 0;
    words.forEach(word => {
      if (malagasyKeywords.includes(word)) mgCount++;
    });
    
    if (mgCount >= 3) return 'mg';
    
    return currentLanguage;
  };
  
  console.log('[Agent Utils] ✅ Ready');
})();

// ==========================================
// 2. MAIN AGENT MIORA ASSISTANT
// ==========================================
(function initAgentMiora() {
  'use strict';

  console.log('[Agent Core] 🤖 Initializing...');
  ...
  // DOM Elements
  const elements = {
    navBtn: document.getElementById('nav-agent'),
    chatWindow: document.getElementById('agent-chat'),
    closeBtn: document.getElementById('agent-close'),
    messagesDiv: document.getElementById('agent-messages'),
    inputField: document.getElementById('agent-input'),
    sendBtn: document.getElementById('agent-send')
  };

  if (!elements.navBtn || !elements.chatWindow || !elements.messagesDiv) {
    console.error('❌ Agent: Missing critical elements');
    return;
  }
  ...
  const state = {
    conversationHistory: [],
    currentLanguage: localStorage.getItem('agent-language') || 'mg',
    currentTheme: localStorage.getItem('agent-theme') || 'purple',
    isDarkMode: localStorage.getItem('agent-dark-mode') === 'true',
    isTyping: false,
    isRecording: false,
    recognition: null,
    synthesis: window.speechSynthesis,
    imageStyle: localStorage.getItem('agent-image-style') || 'professional',
    imageSize: localStorage.getItem('agent-image-size') || 'square',
    notificationsEnabled: localStorage.getItem('agent-notifications') !== 'false',
    soundEnabled: localStorage.getItem('agent-sound') !== 'false'
  };
  ...
  function showNotification(message, type = 'info', duration = 3000) {
    ...
    const notification = document.createElement('div');
    notification.className = `agent-notification agent-notification-${type}`;
    notification.textContent = message;
    ...
    const colors = {
      ...
      warning: '#f59e0b',
      error: '#ef4444',
      info: config.themes[state.currentTheme].primary
    };
  ...
  function applyTheme(themeName) {
    ...
    root.style.setProperty('--agent-primary', theme.primary);
    root.style.setProperty('--agent-secondary', theme.secondary);
    root.style.setProperty('--agent-gradient', theme.gradient);
  ...
  function toggleDarkMode(enabled) {
    ...
    localStorage.setItem('agent-dark-mode', enabled);
    
    if (enabled) {
      document.body.classList.add('agent-dark-mode');
      applyTheme('dark');
    } else {
      document.body.classList.remove('agent-dark-mode');
  ...
  function addMessage(text, isUser = false, imageUrl = null, animate = true) {
    const msgDiv = document.createElement('div');
    msgDiv.className = `agent-message ${isUser ? 'agent-user' : 'agent-bot'}`;
    ...
    if (imageUrl) {
      imageHTML = `
        <div class="agent-message-image">
          <img src="${imageUrl}" alt="Generated Image" onclick="window.agentViewImage('${imageUrl}')" loading="lazy">
        </div>
      `;
    }
    ...
    const avatarHTML = isUser ?
      ...
      `<img src="https://i.ibb.co/fVfNcLv9/file-000000008188722fb075911ad3cee715.png" alt="Agent Miora" class="agent-msg-avatar">`;
    ...
    msgDiv.innerHTML = `
      ...
      <div class="agent-message-content">
        <div class="agent-message-text">${formattedText}</div>
        ${imageHTML}
        <div class="agent-message-actions">
          ${!isUser ? `
            <button class="msg-action-btn" onclick="window.agentCopy(this)" title="Copier">
              <i class="fa-solid fa-copy"></i>
            </button>
            <button class="msg-action-btn" onclick="window.agentSpeak(this)" title="Lire">
              <i class="fa-solid fa-volume-up"></i>
            </button>
          ` : ''}
          <button class="msg-action-btn msg-delete-btn" onclick="window.agentDeleteMessage(this)" title="Supprimer">
            <i class="fa-solid fa-trash"></i>
          </button>
        </div>
      </div>
  ...
  function showTyping() {
    ...
    typingDiv.className = 'agent-message agent-bot agent-typing-msg';
    typingDiv.innerHTML = `
      <div class="agent-message-avatar">
        <img src="https://i.ibb.co/fVfNcLv9/file-000000008188722fb075911ad3cee715.png" alt="Agent Miora" class="agent-msg-avatar">
      </div>
  ...
  window.agentCopy = function(btn) {
    const text = btn.closest('.agent-message-content')
      .querySelector('.agent-message-text').textContent;
  ...
  window.agentSpeak = function(btn) {
    const text = btn.closest('.agent-message-content')
      .querySelector('.agent-message-text').textContent;
    ...
    if (state.synthesis) {
      ...
      utterance.lang = state.currentLanguage === 'mg' ? 'fr-FR' : 
                      state.currentLanguage === 'fr' ? 'fr-FR' : 'en-US';
      state.synthesis.speak(utterance);
    }
  };

  window.agentDeleteMessage = function(btn) {
    if (!confirm('Supprimer ce message?')) return;
    btn.closest('.agent-message').remove();
  };

  window.agentViewImage = function(url) {
    const overlay = document.createElement('div');
    overlay.className = 'agent-image-overlay';
    overlay.innerHTML = `
      <div class="agent-image-viewer">
        <button class="agent-image-close" onclick="this.closest('.agent-image-overlay').remove()">
          <i class="fa-solid fa-xmark"></i>
        </button>
        <img src="${url}" alt="Full image">
        <div class="agent-image-actions">
          <a href="${url}" download class="agent-image-download">
            <i class="fa-solid fa-download"></i> Télécharger
          </a>
        </div>
      </div>
  ...
  try {
    ...
    const cleanPrompt = fullPrompt.replace(/[^\w\s,-]/g, '').trim();
    const encodedPrompt = encodeURIComponent(cleanPrompt);
    const seed = Math.floor(Math.random() * 1000000);
    
    // TRY MULTIPLE APIs - Return first valid URL
    const apis = [
      // API 1: Pollinations primary
      `https://image.pollinations.ai/prompt/${encodedPrompt}?width=${dimensions.width}&height=${dimensions.height}&seed=${seed}&nologo=true`,
      
      // API 2: Unsplash (real photos by keyword)
      `https://source.unsplash.com/${dimensions.width}x${dimensions.height}/?${encodedPrompt.split(',')[0]}&sig=${seed}`,
      
      // API 3: Picsum (random quality photos)
      `https://picsum.photos/${dimensions.width}/${dimensions.height}?random=${seed}`,
      
      // API 4: PlaceIMG (category-based)
      `https://placeimg.com/${dimensions.width}/${dimensions.height}/tech?t=${seed}`,
      
      // API 5: Placeholder (absolute fallback)
      `https://via.placeholder.com/${dimensions.width}x${dimensions.height}/667eea/ffffff?text=${encodedPrompt.slice(0, 30)}`
    ];
    
    // Test each API with timeout
    for (let i = 0; i < apis.length; i++) {
      try {
        const testUrl = apis[i];
        console.log(`[Image] 🔄 Trying API ${i + 1}:`, testUrl);
        
        // Quick test with Image object (not fetch - avoids CORS)
        const img = new Image();
        ...
        if (loaded) {
          console.log(`[Image] ✅ API ${i + 1} works!`);
          return testUrl;
        } else {
          console.log(`[Image] ❌ API ${i + 1} failed`);
        }
        
      } catch (error) {
        console.warn(`[Image] ⚠️ API ${i + 1} error:`, error.message);
        continue;
      }
    }
    
    // If all fail, return last fallback
    console.warn('[Image] ⚠️ All APIs failed, using fallback');
    return apis[apis.length - 1];
    
  } catch (error) {
    console.error('[Image] ❌ Fatal error:', error);
    const dimensions = config.imageSizes[size] || config.imageSizes.square;
    return `https://via.placeholder.com/${dimensions.width}x${dimensions.height}/667eea/ffffff?text=Error`;
  }
}

window.agentGenerateImage = generateImage;
  // ========================================
  // AI API CALL
  // ========================================
  ...
  async function callAI(userMessage, addToHistory = true) {
    try {
      ...
      if (detectedLang !== currentLanguage) {
        ...
        localStorage.setItem('agent-language', detectedLang);
...
💡 **EXPERTISE:**
- **Marketing:** Stratégie digitale, copywriting, SEO
- **Prompt Engineering:** Optimisation prompts AI
- **Culture:** Histoire, sciences, arts
- **Business:** Entrepreneuriat, gestion
- **Tech:** Code, web dev, AI tools
...
💡 **EXPERTISE:**
- **Marketing:** Stratégie digitale, copywriting
- **Prompt Engineering:** Optimisation IA
- **Culture:** Histoire, sciences, arts
...
🎯 **YOUR ROLE**
- Help with **general knowledge** (history, science...)
...
💡 **EXPERTISE:**
- **Marketing:** Digital strategy, copywriting
- **Prompt Engineering:** AI optimization
- **Culture:** History, sciences, arts
...
Respond in **English**.`
      ...
      const SYSTEM_PROMPT = systemPrompts[currentLanguage] || systemPrompts['mg'];
      ...
      showNotification(`⚠️ Erreur: ${error.message}`, 'error');
      return `⚠️ Erreur: ${error.message}`;
    }
  }

  window.agentCallAI = callAI;
  ...
  async function sendMessage() {
    ...
    // Check for image generation - IMPROVED PATTERNS
const imagePatterns = [
  // Malagasy patterns
  /(?:manao|mamorona|mahavita)\s+sary\s+(.+)/i,
  /sary\s+(?:misy|ahitana|ho an'ny)\s+(.+)/i,
  /sary\s+(.+)/i,
  
  // French patterns
  /(?:créer?|génér(?:e|er)|faire|dessine(?:r)?)\s+(?:une?\s+)?(?:image|photo|illustration)\s+(?:de\s+|d'|pour\s+)?(.+)/i,
  /(?:image|photo|illustration)\s+(?:de\s+|d'|avec\s+|pour\s+)?(.+)/i,
  
  // English patterns
  /(?:create|make|generate|draw|design)\s+(?:an?\s+)?(?:image|picture|photo|illustration)\s+(?:of\s+|with\s+|showing\s+)?(.+)/i,
  /(?:image|picture|photo)\s+(?:of\s+|with\s+|showing\s+)?(.+)/i
];
...
for (const pattern of imagePatterns) {
  ...
  if (match && match[1]) {
    ...
    // Make sure it's not too short or just noise
    if (extracted.length > 2) {
      imagePrompt = extracted;
      console.log('[Image Pattern] ✅ Matched:', pattern.source.substring(0, 50) + '...');
      break;
    }
  }
}
    ...
    if (imagePrompt) {
      console.log('[Agent] 🎨 Image generation requested:', imagePrompt);
      
      const msgDiv = addMessage('', false);
      const textDiv = msgDiv.querySelector('.agent-message-text');
      
      textDiv.innerHTML = `
        <div style="color:#fff; padding:16px; background:rgba(139,92,246,0.1); border-radius:10px;">
          <div style="display:flex; align-items:center; gap:12px;">
            <div style="font-size:32px; animation: spin 2s linear infinite;">🎨</div>
            <div>
              <div style="font-size:15px; font-weight:700;">Génération en cours...</div>
              <div style="font-size:12px; opacity:0.8;">"${imagePrompt}"</div>
            </div>
          </div>
        </div>
 ...
 try {
  const imageUrl = await generateImage(imagePrompt, state.imageStyle, state.imageSize);
  
  // AFFICHAGE ROBUSTE SANS VALIDATION
  textDiv.innerHTML = `
    <div style="color:#fff;">
      <div style="padding:12px; background:linear-gradient(135deg, #8b5cf6, #7c3aed); border-radius:10px; margin-bottom:12px; text-align:center;">
        <div style="font-size:16px; font-weight:700;">🎨 Image générée</div>
        <div style="font-size:13px; opacity:0.9; margin-top:4px;">"${imagePrompt}"</div>
      </div>
      
      <div style="position:relative; border-radius:10px; overflow:hidden; margin-bottom:12px; background:linear-gradient(135deg, #1e293b, #334155); padding:4px;">
        <img src="${imageUrl}" 
             alt="${imagePrompt}" 
      ...
      <div style="display:flex; gap:8px; flex-wrap:wrap;">
        <button onclick="window.open('${imageUrl}', '_blank')" 
          style="flex:1; min-width:100px; padding:12px; background:rgba(59,130,246,0.2); color:#60a5fa; border:1px solid #3b82f6; border-radius:8px; text-align:center; font-weight:600; font-size:13px; cursor:pointer; transition:all 0.2s;"
        ...
        <button onclick="
          ...
          link.href = '${imageUrl}';
          link.download = 'miora-${imagePrompt.slice(0,20).replace(/[^a-z0-9]/gi, '_')}-${Date.now()}.png';     link.target = '_blank';
          link.rel = 'noopener noreferrer';
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
          this.innerHTML = '✅ Téléchargement lancé!';
          setTimeout(() => this.innerHTML = '💾 Télécharger', 3000);
        " 
          style="flex:1; min-width:100px; padding:12px; background:rgba(16,185,129,0.2); color:#6ee7b7; border:1px solid #10b981; border-radius:8px; text-align:center; font-weight:600; font-size:13px; cursor:pointer; transition:all 0.2s;"
        ...
        <button onclick="
          navigator.clipboard.writeText('${imageUrl}').then(() => {
            this.innerHTML = '✅ URL copiée!';
            setTimeout(() => this.innerHTML = '📋 Copier URL', 2000);
          }).catch(() => {
            prompt('Copiez ce lien:', '${imageUrl}');
          });
        " 
          style="flex:1; min-width:100px; padding:12px; background:rgba(139,92,246,0.2); color:#c4b5fd; border:1px solid #8b5cf6; border-radius:8px; text-align:center; font-weight:600; font-size:13px; cursor:pointer; transition:all 0.2s;"
      ...
      <div style="margin-top:12px; padding:10px; background:rgba(59,130,246,0.1); border-radius:8px; font-size:12px; color:rgba(255,255,255,0.7); text-align:center;">
        💡 Astuce: Cliquez sur l'image pour l'agrandir
      </div>
    </div>
  `;
  
  console.log('[Image] ✅ Image displayed successfully');
  console.log('[Image] 🔗 Final URL:', imageUrl);
  
} catch (error) {
  textDiv.innerHTML = `
    <div style="padding:20px; background:rgba(239,68,68,0.1); border:2px solid rgba(239,68,68,0.3); border-radius:12px; color:#fca5a5;">
      <div style="font-size:18px; font-weight:700; margin-bottom:12px; text-align:center;">⚠️ Échec de génération</div>
      <div style="font-size:13px; margin-bottom:12px; text-align:center; opacity:0.9;">L'API d'images n'a pas répondu</div>
      <div style="font-size:12px; opacity:0.7; text-align:center; padding:10px; background:rgba(0,0,0,0.2); border-radius:6px;">
        <strong>Détails:</strong> ${error.message}
      </div>
      <div style="margin-top:12px; font-size:11px; text-align:center; opacity:0.6;">
        Essayez avec une description plus simple ou réessayez dans quelques instants
      </div>
    </div>
  `;
  console.error('[Image] ❌ Generation failed:', error);
  showNotification('⚠️ Erreur génération image', 'error', 3000);
}
  ...
  function setupVoiceRecognition() {
    ...
    state.recognition.onstart = () => {
      ...
      showNotification('🎤 Écoute...', 'info', 2000);
    };
    ...
    state.recognition.onerror = (event) => {
      ...
      const errorMessages = {
        'mg': '⚠️ Tsy afaka mihaino',
        'fr': '⚠️ Erreur microphone',
        'en': '⚠️ Microphone error'
      };
      showNotification(errorMessages[state.currentLanguage], 'error');
    };
  ...
  function toggleVoiceRecognition() {
    if (!state.recognition) {
      showNotification('⚠️ Reconnaissance vocale indisponible', 'warning');
      return;
    }
    ...
    if (state.isRecording) {
      ...
      try {
        ...
        showNotification('⚠️ Impossible de démarrer', 'error');
      }
    }
  }
  ...
  function createSettingsPanel() {
    ...
    settingsPanel.id = 'agent-settings-panel';
    settingsPanel.className = 'agent-settings-panel';
    
    settingsPanel.innerHTML = `
      <div class="agent-settings-overlay" onclick="window.agentCloseSettings()"></div>
      <div class="agent-settings-content">
        <div class="agent-settings-header">
          <h3>⚙️ Paramètres</h3>
          <button class="agent-settings-close" onclick="window.agentCloseSettings()">
            <i class="fa-solid fa-xmark"></i>
          </button>
        </div>
        ...
        <div class="agent-settings-body">
          ...
          <div class="setting-section">
            ...
            <div class="language-selector" style="display:flex; gap:10px;">
              <button class="language-btn ${state.currentLanguage === 'mg' ? 'active' : ''}" 
                      data-lang="mg"
                      style="flex:1; padding:12px; background:${state.currentLanguage === 'mg' ? 'linear-gradient(135deg, #10b981, #059669)' : 'rgba(148,163,184,0.2)'}; color:#fff; border:${state.currentLanguage === 'mg' ? '2px solid #10b981' : '1px solid rgba(148,163,184,0.3)'}; border-radius:10px; font-weight:600; cursor:pointer;"
                      onclick="window.agentSwitchLanguage('mg')">
                🇲🇬 Malagasy
              </button>
              <button class="language-btn ${state.currentLanguage === 'fr' ? 'active' : ''}" 
                      data-lang="fr"
                      style="flex:1; padding:12px; background:${state.currentLanguage === 'fr' ? 'linear-gradient(135deg, #3b82f6, #2563eb)' : 'rgba(148,163,184,0.2)'}; color:#fff; border:${state.currentLanguage === 'fr' ? '2px solid #3b82f6' : '1px solid rgba(148,163,184,0.3)'}; border-radius:10px; font-weight:600; cursor:pointer;"
                      onclick="window.agentSwitchLanguage('fr')">
                🇫🇷 Français
              </button>
              <button class="language-btn ${state.currentLanguage === 'en' ? 'active' : ''}" 
                      data-lang="en"
                      style="flex:1; padding:12px; background:${state.currentLanguage === 'en' ? 'linear-gradient(135deg, #8b5cf6, #7c3aed)' : 'rgba(148,163,184,0.2)'}; color:#fff; border:${state.currentLanguage === 'en' ? '2px solid #8b5cf6' : '1px solid rgba(148,163,184,0.3)'}; border-radius:10px; font-weight:600; cursor:pointer;"
                      onclick="window.agentSwitchLanguage('en')">
                🇬🇧 English
              </button>
            </div>
          </div>
              ...
              ${Object.keys(config.themes).map(themeName => {
                ...
                return `
                  <button class="theme-option ${isActive ? 'active' : ''}" 
                          ...
                          style="background: ${theme.gradient}; padding:12px; border-radius:8px; color:white; border:none; cursor:pointer; font-weight:600;">
          ...
          <div class="setting-section">
            ...
            <div class="setting-item">
              <label>
                <input type="checkbox" id="agent-dark-mode-toggle" 
                       ${state.isDarkMode ? 'checked' : ''}
                       onchange="window.agentToggleDarkMode(this.checked)">
          ...
          <div class="setting-section">
            ...
            <div class="setting-item">
              <label>Style:</label>
              <select id="agent-image-style" onchange="window.agentSetImageStyle(this.value)" style="width:100%; padding:8px; background:#1e293b; color:#fff; border:1px solid rgba(148,163,184,0.3); border-radius:6px;">
                ${Object.keys(config.imageStyles).map(style => 
                  `<option value="${style}" ${state.imageStyle === style ? 'selected' : ''}>${style.charAt(0).toUpperCase() + style.slice(1)}</option>`
                ).join('')}
              </select>
            </div>
            <div class="setting-item">
              <label>Taille:</label>
              <select id="agent-image-size" onchange="window.agentSetImageSize(this.value)" style="width:100%; padding:8px; background:#1e293b; color:#fff; border:1px solid rgba(148,163,184,0.3); border-radius:6px;">
                ${Object.keys(config.imageSizes).map(size => 
                  `<option value="${size}" ${state.imageSize === size ? 'selected' : ''}>${config.imageSizes[size].label}</option>`
                ).join('')}
              </select>
            </div>
          </div>
          ...
          <div class="setting-section">
            ...
            <div class="setting-item">
              <label>
                <input type="checkbox" id="agent-notifications" 
                       ${state.notificationsEnabled ? 'checked' : ''}
                       onchange="window.agentToggleNotifications(this.checked)">
                Activer les notifications
              </label>
            </div>
            <div class="setting-item">
              <label>
                <input type="checkbox" id="agent-sound" 
                       ${state.soundEnabled ? 'checked' : ''}
                       onchange="window.agentToggleSound(this.checked)">
                Activer les sons
              </label>
            </div>
          </div>
          ...
          <div class="setting-section">
            <h4>🔧 Actions</h4>
            <button class="setting-action-btn" onclick="window.agentClearHistory()" style="width:100%; padding:10px; background:rgba(239,68,68,0.2); color:#fca5a5; border:1px solid #ef4444; border-radius:8px; cursor:pointer; font-weight:600; margin-top:8px;">
              <i class="fa-solid fa-trash"></i> Effacer l'historique
            </button>
            <button class="setting-action-btn" onclick="window.agentExportConversation()" style="width:100%; padding:10px; background:rgba(59,130,246,0.2); color:#60a5fa; border:1px solid #3b82f6; border-radius:8px; cursor:pointer; font-weight:600; margin-top:8px;">
              <i class="fa-solid fa-download"></i> Exporter conversation
            </button>
            <button class="setting-action-btn" onclick="window.agentResetSettings()" style="width:100%; padding:10px; background:rgba(245,158,11,0.2); color:#fbbf24; border:1px solid #f59e0b; border-radius:8px; cursor:pointer; font-weight:600; margin-top:8px;">
        ...
        <div class="agent-settings-footer">
          <small>Agent Miora v3.0 - Universal AI Assistant</small>
        </div>
      </div>
    `;
    ...
    settingsPanel.querySelectorAll('.theme-option').forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        window.agentSelectTheme(themeName);
      });
    });
  }

  // Settings functions
  window.agentOpenSettings = function() {
    let panel = document.getElementById('agent-settings-panel');
    if (!panel) {
      createSettingsPanel();
      panel = document.getElementById('agent-settings-panel');
    }
    panel.classList.add('show');
  };

  window.agentCloseSettings = function() {
    const panel = document.getElementById('agent-settings-panel');
    if (panel) panel.classList.remove('show');
  };

  window.agentSelectTheme = function(themeName) {
  ...
  window.agentToggleDarkMode = function(enabled) {
    toggleDarkMode(enabled);
  };

  window.agentToggleNotifications = function(enabled) {
    state.notificationsEnabled = enabled;
    localStorage.setItem('agent-notifications', enabled);
    showNotification(enabled ? '🔔 Notifications activées' : '🔕 Notifications désactivées', 'info', 1500);
  };

  window.agentToggleSound = function(enabled) {
    state.soundEnabled = enabled;
    localStorage.setItem('agent-sound', enabled);
    if (enabled) playSound('success');
  };

  window.agentSwitchLanguage = function(lang) {
  ...
  window.agentSetImageStyle = function(style) {
    state.imageStyle = style;
    localStorage.setItem('agent-image-style', style);
    showNotification(`🎨 Style: ${style}`, 'success', 1500);
  };

  window.agentSetImageSize = function(size) {
    state.imageSize = size;
    localStorage.setItem('agent-image-size', size);
    showNotification(`📐 Taille: ${config.imageSizes[size].label}`, 'success', 1500);
  };

  window.agentClearHistory = function() {
    ...
    const messages = elements.messagesDiv.querySelectorAll('.agent-message:not(:first-child)');
    messages.forEach(msg => msg.remove());
    
    showNotification('🗑️ Historique effacé', 'info');
  };

  window.agentExportConversation = function() {
    const data = {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `agent-miora-chat-${Date.now()}.json`;
    a.click();
    URL.revokeObjectURL(url);
  ...
  window.agentResetSettings = function() {
    if (!confirm('Réinitialiser tous les paramètres?')) return;

    localStorage.removeItem('agent-theme');
    localStorage.removeItem('agent-language');
    localStorage.removeItem('agent-dark-mode');
    localStorage.removeItem('agent-notifications');
    localStorage.removeItem('agent-sound');
    localStorage.removeItem('agent-image-style');
    localStorage.removeItem('agent-image-size');
  ...
  function createAdditionalUI() {
    const header = elements.chatWindow.querySelector('.agent-header');
    if (!header) return;

    // Settings Button
    const settingsBtn = document.createElement('button');
    settingsBtn.className = 'agent-header-btn';
    settingsBtn.innerHTML = '<i class="fa-solid fa-gear"></i>';
    settingsBtn.title = 'Paramètres';
    settingsBtn.onclick = window.agentOpenSettings;

    // Export Button
    const exportBtn = document.createElement('button');
    exportBtn.className = 'agent-header-btn';
    exportBtn.innerHTML = '<i class="fa-solid fa-download"></i>';
    exportBtn.title = 'Exporter';
    exportBtn.onclick = window.agentExportConversation;

    // Clear Button
    const clearBtn = document.createElement('button');
    clearBtn.className = 'agent-header-btn agent-clear-btn';
    clearBtn.innerHTML = '<i class="fa-solid fa-trash-can"></i>';
    clearBtn.title = 'Effacer';
    clearBtn.onclick = window.agentClearHistory;

    const closeBtn = header.querySelector('.agent-close');
    header.insertBefore(settingsBtn, closeBtn);
    header.insertBefore(exportBtn, closeBtn);
    header.insertBefore(clearBtn, closeBtn);

    // Mic Button
    const inputArea = elements.chatWindow.querySelector('.agent-input-area');
    if (inputArea) {
...
**What can I help you with today?** 🚀`
  ...
  const msgDiv = document.createElement('div');
  msgDiv.className = 'agent-message agent-bot';
  msgDiv.innerHTML = `
    <div class="agent-message-avatar">
      <img src="https://i.ibb.co/fVfNcLv9/file-000000008188722fb075911ad3cee715.png" alt="Agent Miora" class="agent-msg-avatar">
    </div>
    <div class="agent-message-content">
      <div class="agent-message-text">${formatText(welcomeMsg)}</div>
    </div>
  `;
  
  elements.messagesDiv.appendChild(msgDiv);

  console.log('[Agent Core] ✅ Fully initialized!');
  console.log('[Agent Core] 🌐 Language:', state.currentLanguage);
  console.log('[Agent Core] 🎨 Theme:', state.currentTheme);

  showNotification('✨ Agent Miora prêt!', 'success', 2000);
...
window.addEventListener('error', function(event) {
  ...
  if (event.error && event.error.stack && event.error.stack.includes('agent')) {
    console.error('🚨 [Agent Error Stack]:', event.error.stack);
...
console.log('✅ [Agent Miora] Error handlers installed');
console.log('🎉 [Agent Miora] System ready!');